function nexport_ML(nexon, DFID_postOp, DFID_preOp,  labelIDs, exportFcn, args)
    % export each trial-sample to an N-fold machine learning dataset as:
    % index (with generated labels)
    % folds containing samples
    % use modality-specific functions to generate labels

    % CFG HEADER
    n_folds = args.n_folds; % default = 5    

    n_dtsRows = height(nexon.console.BASE.DTS);    
    ...
    for i = 1:n_dtsRows
        ...
        exportFcn(nexon, DF_target, DF_source, [], fid_index, foldDir)
    end
end